...
function CreateGroup({ handleCloseCreateGroup, handleSearch }) {
   const [searchMember, setSearchMember] = useState("");
   const [newGroup, setNewGroup] = useState(false)
   const [groupMember, setGroupMember] = useState(new Set()); // Set() store only unique value's 
   ...
   const handleNewGroup = () => {
      newGroup ? setNewGroup(false) : setNewGroup(true);
   }
         ...
         {!newGroup && <>
            <div className='w-[50%]'>
               <div className='flex flex-row space-x-10 bg-[#008069] text-white p-4'>
                  <BsArrowLeft className='mt-px cursor-pointer text-3xl' onClick={handleCloseCreateGroup} />
            ...
            <div className='w-[50%]'>
               ...
               <button className='absolute right-[10%] bottom-[10%] cursor-pointer rounded-full font-semibold border-green-600 border-2 text-green-600 hover:bg-green-600 hover:text-white py-2 px-4' onClick={handleNewGroup}>Create Group With This Members</button>
            </div>
         </>}
      </div>
   )
