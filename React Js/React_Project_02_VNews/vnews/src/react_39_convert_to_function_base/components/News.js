...
const News = (props) => {
   ...
   // Set up fetchData to run whenever the page state updates
   useEffect(() => {
      if (page > 1) { // Ensures this runs only when page is greater than 1 (i.e., not on initial load)
         fetchMoreData();
      }
   }, [page]); // Run this effect only when the 'page' changes

   // Function to update the page
   const handlePageUpdate = () => {
      setPage((prevPage) => prevPage + 1); // Just update the page here
   };

   const fetchMoreData = async () => {
      console.log(page); // Logs the current page number after updating

      let url = `${mainURl}&page=${page}&pageSize=${props.pageSize}`; // Use the updated page state directly
      console.log(url);
