<script>
   ...
   async function pinNote(note) {
      ...
      try {
         ...
         console.error("Error where pin/un-pin note:", error);
         softAlert("danger", "Error when trying to pin/un-pin note ", 30000);
      }
   }
      ...
      try {
         ...
         if (!response.ok) {
            if (response.status === 404) { // Specific check for 404
               ...
               // Handle "No notes found" as a special case
               document.getElementById('notesContainer').innerHTML = "<p>" + errorText + "</p>"; // No error class needed
               throw new Error(response.status); // Stop further processing, no need to throw an error
            } else {
               // For other errors (not 404), throw the error to be caught below
               const errorText = await response.text();
               throw new Error(errorText);
   ...
   $("#addNoteSubmit").submit(async function (event) {
      ...
      try {
         ...
         // Make AJAX request
         // this return note (here response is note)
         ...
         console.log("Add New Note at endpoint:- " + "/api/note/addNote", noteData);

         // const note = await response.json();
         // console.log(response);
         if (response !== null) {
            softAlert("success", "Note add successfully.", 2000);
            ...
            createNoteCard(response);
         } else {
            softAlert("danger", "Note not created due to same error", 30000);
         }
      } catch (error) {
   ...
   async function editNote(id) {
      ...
      try {
         ...
         const note = await getThisNote(id);
